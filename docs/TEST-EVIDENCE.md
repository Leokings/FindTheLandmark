...
- 2026-09-26 [isolated StudioNet retest `KWKW6B`](https://find-the-landmark.vercel.app/?results=KWKW6B) on identical v4.6 code at [`0x5BEB...05F4`](https://explorer-studio.genlayer.com/address/0x5BEB00706a08B9B104bBE021f2021B1E0e0b05F4): 30 players confirmed 360 answers; 12/12 rounds settled with no voids. The corrected GenLayer decorator round 7 scored all 30 answers and awarded 674 XP. Round 4 revealed only 7/30 answers, leaving 23 unscored (including six correct selections). The app and finalized onchain leaderboards both contain 30 players and 9,017 total XP. This is **not** a clean full-participation pass.
- 2026-09-26 backend v43 queues signed reveal details before receipt finality, opens the reveal window earlier, and counts the contract's actual accepted reveals. Existing answer rows were migrated; 26 app tests and the live results smoke check pass. A new 30-player StudioNet scoring pass has **not** yet been run for v43.
- The first retest room `KSHKJN` was stopped before round 1 when the shared production StudioNet contract hit its daily per-contract snapshot-storage budget; the isolated contract avoided changing the live app's default address.
- For `WMSESY`, the finished app and finalized onchain leaderboards each contain 30 players and 9,248 total XP. Winner: Load 01 with 555 XP. Near round 10, StudioNet briefly returned its 30-requests-per-minute limit; all 30 commitments eventually landed, but that burst took 79 seconds of the 90-second window.
...
- Build, lint, contract lint, 16 direct contract tests, the StudioNet deployment policy test, and 26 app tests pass. A 30-active-player StudioNet match and 12/12 round settlement are verified, but 30/30 scoring in every round is **not**.
